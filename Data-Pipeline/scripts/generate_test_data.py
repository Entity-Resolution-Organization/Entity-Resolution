...
def preprocess_and_save(raw_df, dataset_config: dict, dataset_name: str, output_dir: str):
    ...
    # Save processed data
    accounts_path = os.path.join(output_dir, 'processed', f'{dataset_name}_accounts.csv')
...
def main():
    parser = argparse.ArgumentParser(description='Generate test data for entity resolution pipeline')
    parser.add_argument('--dataset', type=str, default='pseudopeople',
                        choices=['pseudopeople', 'nc_voters', 'ofac_sdn', 'wdc_products', 'amazon_2018', 'dblp_acm'],
    ...
    if args.all:
        datasets = ['pseudopeople', 'nc_voters', 'ofac_sdn', 'wdc_products', 'amazon_2018', 'dblp_acm']
    else:
    ...
    for dataset_name in datasets:
        try:
            raw_df, dataset_config = generate_dataset(dataset_name, config, args.output_dir)
            metrics = preprocess_and_save(raw_df, dataset_config, dataset_name, args.output_dir)
    ...
    for m in all_metrics:
        ...
        print(f"  Training pairs: {m['total_pairs']} (pos: {m['positive_pairs']}, neg: {m['negative_pairs']})")
