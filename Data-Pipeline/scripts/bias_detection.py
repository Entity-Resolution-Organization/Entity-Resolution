...
class BiasDetector:
    ...
    def analyze_entity_type_distribution(self, df: pd.DataFrame,
                                          type_col: str = 'entity_type') -> Dict[str, Any]:
        """
        ...
        if len(distribution) == 0:
            return {
                'status': 'skipped',
                'reason': 'No entity types found'
            }

        imbalance = float(distribution.max() - distribution.min()) if len(distribution) > 1 else 0.0
        ...
        return {
            ...
            'recommendation': 'Oversample minority entity types or add more diverse data sources' if imbalance >= 0.3 else 'Distribution is acceptable'
        }
    ...
    def _infer_entity_types(self, df: pd.DataFrame) -> Optional[Dict]:
        ...
        def classify_name(name):
            ...
            # Company indicators
            company_patterns = ['inc', 'llc', 'ltd', 'corp', 'company', 'co.', 'gmbh', 'ag']
        ...
        imbalance = (distribution.max() - distribution.min()) if len(distribution) > 1 else 0.0
    ...
    def analyze_language_bias(self, df: pd.DataFrame,
                               name_col: str = 'name') -> Dict[str, Any]:
        """
        ...
        if name_col not in df.columns:
            return {
                'status': 'skipped',
                'reason': f'No {name_col} column found'
            }
        ...
        def analyze_name_charset(text):
            ...
            has_latin_extended = bool(re.search(r'[\u00c0-\u024f]', text))  # Accented chars
        ...
        total = len(df)
        non_ascii = total - charset_counts.get('ascii_only', 0) - charset_counts.get('empty', 0)
        ...
        return {
            ...
            'severity': 'HIGH' if non_ascii_pct < 2 else 'MEDIUM' if non_ascii_pct < 5 else 'LOW',
            'recommendation': 'Add multi-lingual datasets (OFAC international, Rakuten Japan, EU company registers)' if non_ascii_pct < 5 else 'Language diversity is acceptable'
        }

    def analyze_geographic_bias(self, df: pd.DataFrame,
                                 address_col: str = 'address') -> Dict[str, Any]:
        """
        ...
        if address_col not in df.columns:
            return {
                'status': 'skipped',
                'reason': f'No {address_col} column found'
            }
        ...
        total_known = len(df) - region_counts.get('unknown', 0) - region_counts.get('other', 0)
        ...
        return {
            ...
            'recommendation': 'Add international datasets (GLEIF global companies, OpenAddresses international)' if us_pct > 80 else 'Geographic diversity is acceptable'
        }

    def analyze_match_label_distribution(self, pairs_df: pd.DataFrame,
        ...
        if label_col not in pairs_df.columns:
            return {
                'status': 'skipped',
                'reason': f'No {label_col} column found'
            }
        ...
        return {
            ...
            'severity': 'HIGH' if imbalance > 30 else 'MEDIUM' if imbalance > 15 else 'LOW',
            'recommendation': f'Adjust pair sampling ratio (current: {positive_pct:.0f}/{negative_pct:.0f}, target: 50/50)' if imbalance >= 15 else 'Label distribution is acceptable'
        }

    def analyze_data_source_bias(self, df: pd.DataFrame,
                                  id_col: str = 'id') -> Dict[str, Any]:
        """
        ...
        if id_col not in df.columns:
            return {
                'status': 'skipped',
                'reason': f'No {id_col} column found'
            }
        ...
        return {
            ...
            'severity': 'HIGH' if synthetic_pct > 90 else 'MEDIUM' if synthetic_pct > 70 else 'LOW',
            'recommendation': 'Incorporate more real-world datasets (NC Voters, OFAC SDN, WDC Products)' if synthetic_pct > 70 else 'Source balance is acceptable'
        }

    def generate_bias_report(self, accounts_df: pd.DataFrame,
                              pairs_df: pd.DataFrame = None) -> Dict[str, Any]:
        """
        ...
        report['analyses']['entity_type_bias'] = self.analyze_entity_type_distribution(accounts_df)
        report['analyses']['language_bias'] = self.analyze_language_bias(accounts_df)
        report['analyses']['geographic_bias'] = self.analyze_geographic_bias(accounts_df)
        report['analyses']['data_source_bias'] = self.analyze_data_source_bias(accounts_df)
        ...
        if pairs_df is not None and len(pairs_df) > 0:
            ...
            report['analyses']['match_label_bias'] = self.analyze_match_label_distribution(pairs_df)
        ...
        report['summary'] = {
            ...
            'overall_bias_risk': 'HIGH' if high_risk >= 2 else 'MEDIUM' if high_risk >= 1 or medium_risk >= 2 else 'LOW'
        }
        ...
        logger.info(f"[Bias Detection] Overall risk: {report['summary']['overall_bias_risk']}")
    ...
    def print_summary(self, report: Dict[str, Any]) -> None:
        ...
        print(f"Records analyzed: {report['total_accounts']} accounts, {report['total_pairs']} pairs")
        print("-" * 70)
        ...
        for analysis_name, results in report['analyses'].items():
            ...
            if 'positive_percentage' in results:
                print(f"   Label split: {results['positive_percentage']:.0f}% pos / {results['negative_percentage']:.0f}% neg")
            if 'synthetic_percentage' in results:
                print(f"   Synthetic data: {results['synthetic_percentage']:.1f}%")

            if 'recommendation' in results and results.get('severity') in ['HIGH', 'MEDIUM']:
                print(f"   Recommendation: {results['recommendation']}")

        print("\n" + "-" * 70)
        summary = report['summary']
        risk_icon = "🔴" if summary['overall_bias_risk'] == 'HIGH' else "🟡" if summary['overall_bias_risk'] == 'MEDIUM' else "🟢"
        print(f"OVERALL BIAS RISK: {risk_icon} {summary['overall_bias_risk']}")
        print(f"Issues found: {summary['total_issues']} ({summary['high_risk_count']} high, {summary['medium_risk_count']} medium)")
